...
import { FragmentExplosion, OnTarget } from "@atbs/shared-data";
...
function teardownExplosionFixture(tempDir: string | undefined, imageManager: ImageManager | undefined) {
    if (imageManager) {
...
function createMockUnit(
    ...
    (game as unknown as { sides: { id: string; units: Unit[]; oppositionSideIds: string[] }[] }).sides =
        [{ id: "side-1", units: [unit], oppositionSideIds: [] }];
